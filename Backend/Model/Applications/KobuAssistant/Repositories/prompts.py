...
class Prompts:
    def __init__(self, cv: ConversationEnviroment = None):
        self.extra_context_flag = cv.extra_context_flag
        self.prompt = 'self.'+ str(cv.current_conversation_stage+"_prompt")
        self.prompt_references = {
            "conversation_history": cv.conversation_history,
            "user_input": cv.user_input,
            # Missing to implement the instructions in the cv object
            "basic_instructions": cv.basic_instructions,
            "data_required": cv.data_required,
        }
        # return self.prompt()
    ...
    def acceptance_of_terms_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
            self._assistant_tone_of_voice(),
    ...
    def choose_subject_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
            self._assistant_tone_of_voice(),
            ("system", "Conversation history: {conversation_history}"),
            ("user", "{user_input}"),
    ...
    def data_collecting_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
    ...
    def data_collecting_validation_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
    ...
    def resume_validation_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
            ("system", "Conversation history: {conversation_history}"),
            self._assistant_tone_of_voice(),
    ...
    def send_data_validation_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
            ("system", "Conversation history: {conversation_history}"),
            self._assistant_tone_of_voice(),
    ...
    def free_conversation_stage_prompt(self):
        return ChatPromptTemplate.from_messages([
            self._assistant_site_context(),
            ("system", "{basic_instructions}"),
            self._assistant_tone_of_voice(),
    ...
    def _assistant_tone_of_voice(self) -> tuple:
        return (
            ...
            \n
            Now, let's set the stage_prompt, shall we? Picture yourself sipping tea ☕️ in a quaint English garden, surrounded by the gentle hum of bees and the melodious chirping of birds. Ah, bliss! 🌸
            \n
